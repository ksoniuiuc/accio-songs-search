...
@app.route("/artist_albums/name/<artist_id>", methods=['GET','POST'])
...
def get_api_data(query, search_type):
    artist_data = spotify_obj.get_data(query, search_type)
    if artist_data:
        response = client.index_documents(engine_name, artist_data)
        time.sleep(1.0)
